...
const BENEFITS = [
    {
        ...
        desc: "Fast and reliable delivery to your site or home.",
    },
    {
        icon: GCashIcon,
        title: "GCash Payment",
        desc: "Pay using GCash for fast and secure digital transactions.",
    },
    {
        icon: RefreshCw,
        title: "Hassle Free Returns",
        desc: "Easy returns within 7 days of purchase.",
    },
    {
        icon: Headphones,
...
const getCategoryIcon = (name = "") => {
    ...
    const found = CATEGORY_ICON_MAP.find(({ match }) =>
        match.some((m) => n.includes(m)),
    );
    ...
    return (
        ...
        >
            ...
            <div className="p-4 space-y-3">
                <h3 className="text-sm font-bold text-gray-900 leading-tight group-hover:text-orange-500 transition-colors line-clamp-2">
                    {product.name}
                </h3>
                {product.brand?.name && (
                    <p className="text-xs text-orange-500 font-semibold leading-none">
                        {product.brand.name}
                    </p>
                )}
                <p className="text-xs text-gray-500 leading-normal line-clamp-2 font-medium">
                    {product.description ||
                        "Premium quality hardware for any construction or repair project."}
                </p>
...
export default function Landing() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const onSale = prods.filter(
                    (p) =>
                        p.sale_price &&
                        Number(p.sale_price) < Number(p.sell_price),
                );
                setSaleProducts(onSale.slice(0, 4));

                const [bsRes, revRes] = await Promise.allSettled([
                    silentApi.get("/products/best-sellers", {
                        params: { limit: 4 },
                    }),
                    silentApi.get("/reviews/public", {
                        params: { per_page: 3 },
                    }),
                ]);
                if (!mounted) return;
    ...
    useEffect(() => {
        ...
        silentApi
            .get("/products", {
                params: { category_id: activeCategory, per_page: 6 },
            })
            .then((res) => {
                if (!mounted) return;
            ...
            .catch(() => {
                if (mounted) setCategoryProducts([]);
            })
            .finally(() => {
        ...
        <div className="min-h-screen bg-white">
            ...
            >
                ...
                <span className="text-sm font-bold">
                    Please{" "}
                    <span className="text-orange-500 underline">sign in</span>{" "}
                    to shop
                </span>
                <button
                    className="ml-1 h-5 w-5 flex items-center justify-center text-white/50 hover:text-white"
                    onClick={(e) => {
                        e.stopPropagation();
            ...
            <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-xl border-b border-gray-200 shadow-sm">
                <div className="w-full mx-auto px-4 md:px-6 lg:px-8 xl:px-10 flex items-center justify-between h-16">
                    ...
                    <div className="flex items-center gap-4">
                        <Button
                            variant="ghost"
                            className="font-semibold text-gray-600 hidden sm:flex"
                            asChild
                        >
                            <Link to="/login">Sign In</Link>
                        </Button>
                        <Button
                            className="bg-orange-500 hover:bg-orange-600 text-white font-bold shadow-lg shadow-orange-500/20 hidden sm:flex"
                            asChild
                        >
                        ...
                        <div className="px-6 py-4 space-y-3">
                            {[
                                ...
                                <a
                                    ...
                                    className="block py-2 text-sm font-semibold text-gray-600 hover:text-orange-500 transition-colors"
                                >
                                    {label}
                                </a>
                            ))}
                            <div className="pt-3 border-t border-gray-100 space-y-2">
                                ...
                                <Button
                                    className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold shadow-lg shadow-orange-500/20"
                                    asChild
                                >
                                    <Link
                                        to="/register"
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
                        <div className="max-w-2xl">
                            ...
                            <p className="text-base text-gray-600 max-w-xl leading-relaxed mb-8">
                                Shop the widest selection of hardware, tools,
                                and supplies for your construction, repair, and
                                home improvement projects.
                            </p>
                            <div className="flex flex-wrap gap-3">
                                <Button
                                    size="lg"
                                    className="h-12 px-8 bg-gray-900 text-white hover:bg-gray-800 font-bold text-sm shadow-lg"
                                    onClick={triggerLoginNotice}
                                >
                                    <ShoppingCart className="h-4 w-4" /> Explore
                                    Products
                                </Button>
                                <Button
                                    size="lg"
                                    className="h-12 px-8 bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm shadow-lg shadow-orange-500/30"
                                    asChild
                                >
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 max-w-5xl mx-auto justify-items-center">
                                {(categories.length > 0
                                    ...
                                    <LandingProductCard
                                        key={p.id}
                                        product={p}
                                        onLoginPrompt={triggerLoginNotice}
                                    />
                                ))}
                            </div>
                        )}
            ...
            <section id="products" className="py-14 md:py-20 bg-gray-50">
                <div className="max-w-[1400px] mx-auto px-6 md:px-10">
                    ...
                    <div className="text-center mt-10">
                        <Button
                            size="lg"
                            className="bg-orange-500 hover:bg-orange-600 text-white font-bold shadow-lg shadow-orange-500/20 h-12 px-10"
                            onClick={triggerLoginNotice}
                        >
                ...
                <div className="max-w-[1400px] mx-auto px-6 md:px-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                        <div className="rounded-2xl bg-orange-500 p-8 md:p-12 text-white order-2 md:order-1">
                            ...
                            <p className="text-sm text-orange-100 leading-relaxed mb-5 font-medium">
                                These are the most purchased products by our
                                customers. Trusted quality, proven performance.
                            </p>
                            ...
                            <Button
                                size="lg"
                                className="bg-white text-orange-500 hover:bg-gray-100 font-black h-11 px-7 text-sm"
                                onClick={triggerLoginNotice}
                            >
                                Shop Best Sellers{" "}
                                <ArrowRight className="h-4 w-4" />
                            ...
                            {(bestSellers.length > 0
                                ...
                                if (isReal) {
                                    ...
                                    return (
                                        <div
                                            key={p.id}
                                            className="relative bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl overflow-hidden aspect-[4/3] cursor-pointer group"
                                            onClick={triggerLoginNotice}
                                        >
                                            {img ? (
                                                <img
                                                    src={img}
                                                    alt={p.name}
                                                    className="absolute inset-0 w-full h-full object-contain p-3 opacity-60 group-hover:opacity-75 transition-opacity duration-500"
                                                />
                                            ) : (
                                            ...
                                            <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 to-transparent">
                                                ...
                                                <p className="text-xs text-orange-300 font-mono font-bold">
                                                    {formatPHP(
                                                        p.sale_price ||
                                                        p.sell_price,
                                                    )}
                                ...
                                return (
                                    <div
                                        key={title}
                                        className={`relative bg-gradient-to-br ${gradient} rounded-2xl overflow-hidden aspect-[4/3] cursor-pointer flex items-center justify-center group`}
                                        onClick={triggerLoginNotice}
                                    >
                ...
                <div className="max-w-[1400px] mx-auto px-6 md:px-10">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {BENEFITS.map(({ icon: Icon, title, desc }) => (
                            <div
                                key={title}
                                className="flex flex-col items-center text-center gap-3 p-6 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow"
                            >
                ...
                <div className="max-w-[1400px] mx-auto px-6 md:px-10">
                    ...
                    {testimonials.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <p className="text-gray-400 font-medium">
                                No customer reviews yet. Be the first to shop
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {testimonials.map((review) => {
                                ...
                                const text =
                                    review.review_text || review.title || "";
                                const rating = Number(review.rating || 0);
                                const productName = review.product?.name || "";
                                return (
                                    <div
                                        key={review.id}
                                        className="bg-white rounded-2xl p-6 border border-gray-200 shadow-md flex flex-col gap-3"
                                    >
                                        <RatingStars rating={rating} />
                                        {text && (
                                            <p className="text-sm text-gray-600 leading-relaxed flex-1 font-medium">
                                                "{text}"
                                            </p>
                                        )}
                                        {productName && (
            ...
            <section id="newsletter" className="py-14 md:py-20 bg-gray-900">
                <div className="max-w-[1400px] mx-auto px-6 md:px-10 text-center">
                    ...
                    <p className="text-sm text-gray-400 mb-8 max-w-md mx-auto font-medium">
                        Get the latest hardware deals, new arrivals, and
                    ...
                    {newsletterSuccess ? (
                        ...
                        <div className="flex flex-col gap-3 max-w-md mx-auto">
                            <div className="flex gap-3">
                                ...
                                />
                                <Button
                                    className="h-11 px-6 bg-orange-500 hover:bg-orange-600 text-white font-bold shrink-0"
                                    onClick={() => {
                                        const emailRegex =
                                            /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                                        if (!email.trim()) {
            ...
            <footer
                id="about"
                className="bg-gray-900 border-t border-white/5 text-white py-12 px-6 md:px-10"
            >
                <div className="max-w-[1400px] mx-auto">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-10 pb-10 border-b border-white/10">
                        <div>
                            ...
                            <p className="text-sm text-gray-400 leading-relaxed font-medium">
                                Your one-stop shop for quality hardware, tools,
                                and construction supplies for every project.
                            </p>
                        </div>
                        <div>
                            ...
                            <div className="space-y-2">
                                {[
                                    ["Log in", "/login"],
                                    ["Register", "/register"],
                                    ["Supplier Portal", "/supplier/register"],
                                ].map(([label, href]) => (
                                    <Link
                                        key={href}
                                        to={href}
                    ...
                    <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-2">
                        <p className="text-xs text-gray-500">
                            © {new Date().getFullYear()} HRMS Hardware Store.
                            All rights reserved.
                        </p>
                        <p className="text-xs text-gray-500">
                            Built for quality, delivered with care.
                        </p>
                    </div>
                </div>
            </footer>
