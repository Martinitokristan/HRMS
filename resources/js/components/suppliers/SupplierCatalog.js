...
export default function SupplierCatalog() {
    ...
    const handleOrder = async () => {
        ...
        if (orderQty > currentStock) {
            showToast(
                `Only ${currentStock} units available for this option.`,
                "error",
            );
            return;
        }
                ...
                items: [
                    {
                        ...
                        supplier_product_variant_id: selectedVariant
                            ? selectedVariant.id
                            : null,
                        quantity: orderQty,
                        unit_cost: unitCost,
            ...
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3 relative z-0">
                <div>
                    <h2 className="text-xl font-bold text-foreground tracking-tight flex items-center gap-2">
                        <Store className="h-6 w-6 text-primary" /> Supplier
                        Available Products
                    </h2>
                    <p className="text-sm text-muted-foreground mt-0.5">
                        Browse products promoted by your suppliers. Create POs
                        directly from here.
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <Card className="text-center py-16 px-8">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Suppliers haven't added any products yet.
                    </p>
                </Card>
            ) : (
                    ...
                    {products.data.map((p) => (
                        ...
                        >
                            ...
                            <div className="p-4">
                                ...
                                {p.brand && (
                                    <div className="text-[11px] font-semibold text-orange-500 mb-0.5">
                                        {p.brand.name}
                                    </div>
                                )}
                                ...
                                <div className="text-[12px] text-muted-foreground mb-2">
                                    {p.category?.name || "Uncategorized"} &bull;{" "}
                                ...
                                <div className="mb-3">
                                    ...
                                    >
                                        {p.total_stock > 0 ||
                                        p.inventory?.current_stock > 0
                                            ? `✓ ${p.total_stock || p.inventory?.current_stock} Available`
                                            : "✗ Out of Stock"}
                                    </Badge>
                                </div>
                ...
                {viewProduct &&
                    (() => {
                        const variantsSource =
                            viewProduct.product_variants ||
                            viewProduct.variants ||
                            [];
                        const variants = (
                            Array.isArray(variantsSource) ? variantsSource : []
                        ).map((v) => ({
                            id: v.id,
                        ...
                        const currentPrice = selectedVariant
                            ...
                            : viewProduct.price || viewProduct.purchase_price;
                        const currentStock = selectedVariant
                            ? selectedVariant.stock
                            : viewProduct.total_stock ||
                              viewProduct.inventory?.current_stock ||
                              0;
                        ...
                        const mainImage = selectedVariant
                            ...
                            : viewProduct.image_path;
                        const additionalImages = selectedVariant
                            ? selectedVariant.additional_images || []
                            : viewProduct.additional_images || [];
                        const allImages = [
                            mainImage,
                            ...additionalImages,
                        ].filter(Boolean);
                        ...
                        return (
                            <div className="flex flex-col md:flex-row overflow-hidden bg-white text-gray-900 border-none shadow-none">
                                        ...
                                        <div className="flex flex-wrap justify-center gap-3">
                                            {allImages.map((img, i) => (
                                                <button
                                                    ...
                                                    className={`w-14 h-14 rounded-xl border-2 transition-all overflow-hidden bg-white flex items-center justify-center p-1 ${
                                                        displayImage === img
                                ...
                                <div className="md:w-[50%] p-8 flex flex-col overflow-y-auto max-h-[85vh]">
                                    ...
                                    <div className="mb-4">
                                        <div
                                            className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-[10px] font-bold ${
                                                isOutOfStock
                                                ...
                                                {(Array.isArray(variants)
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`px-4 py-1.5 rounded-lg border-2 text-xs font-bold transition-all ${
                                                                isOOS
                                                                    ? "opacity-40 cursor-not-allowed bg-gray-100 border-transparent text-gray-400"
                                                                    : isSelected
                                                                      ? "bg-[#FF5A1F] text-white border-[#FF5A1F] shadow-md shadow-orange-200 scale-105"
                                                                      : "bg-white text-gray-700 border-gray-100 hover:border-orange-200"
                                                            }`}
