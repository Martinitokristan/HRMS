...
const SupplierStatusBadge = ({ status }) => {
    const config = {
        active: {
            className:
                "border-success/30 bg-success-light text-success-foreground",
            label: "Active",
        },
        pending: {
...
export default function Suppliers() {
    ...
    const performDelete = async (id) => {
        ...
        try {
            ...
            if (data && (res.data.status === "success" || !res.data.status)) {
                ...
                showToast(
                    res.data.message || "Error deleting supplier",
                    "error",
                );
            }
        } catch (error) {
    ...
    const stats = useMemo(
        () => ({
            total: total,
            active: (Array.isArray(suppliers) ? suppliers : []).filter(
                (s) => (s.status || "active") === "active",
            ).length,
            pending: (Array.isArray(suppliers) ? suppliers : []).filter(
                (s) => s.status === "pending",
            ).length,
            withProducts: (Array.isArray(suppliers) ? suppliers : []).filter(
                (s) => (s.products_count || 0) > 0,
            ).length,
        }),
        [suppliers, total],
    );
        ...
        <div>
            ...
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                <div>
                    <h2 className="text-xl font-bold text-foreground tracking-tight flex items-center gap-2">
                        <Building2 className="h-6 w-6 text-primary" /> Supplier
                        Management
                    </h2>
                    <p className="text-sm text-muted-foreground mt-0.5">
                        Manage your vendor partnerships and track supplier
                        performance
                    </p>
                </div>
            </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    {[
                        ...
                        },
                        {
                            key: "inactive",
                            label: "Inactive",
                            count: stats.total - stats.active - stats.pending,
                        },
                    ].map(({ key, label, count }) => (
                        <Button
                            key={key}
            ...
            <div className="flex items-center justify-between mb-2 px-1">
                <span className="text-sm text-muted-foreground">
                    Showing {suppliers?.length || 0} of {total} suppliers
                </span>
                {loading && (
