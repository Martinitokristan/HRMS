...
export default function ProductForm({
    ...
    const LEGACY_VARIANT_IDS = [1, 2, 3];
    ...
    const getValuesForVariant = (variantId) => {
        const values = variantValues[variantId] || [];
        if (!form.category_id) return values;

        // Keep category-specific values and global values.
        return values.filter((v) => {
            if (
                ...
                return true;
            return String(v.category) === String(form.category_id);
        });
    };

    const resolvedCategoryVariantTypes = useMemo(() => {
        const mapped = (categoryVariantTypes || [])
            .map((cvt) => {
                const variantId = Number(cvt?.variant_id ?? cvt?.variant?.id);
                if (!variantId) return null;

                const variant =
                    cvt?.variant ||
                    variantTypes.find((vt) => Number(vt.id) === variantId);
                if (!variant) return null;

                return {
                    variant_id: variantId,
                    variant,
                    is_required: Boolean(cvt?.is_required),
                };
            })
            .filter(Boolean);
        ...
        // Fallback 1: infer types from category-tagged values when category mapping rows are absent.
        const inferredByValues = (variantTypes || [])
            .filter((vt) => {
                const values = variantValues[vt.id] || [];
                return values.some(
                    (v) =>
                        String(v?.category || "") ===
            ...
            .map((vt) => ({
                variant_id: Number(vt.id),
                variant: vt,
                is_required: false,
            }));

        if (inferredByValues.length > 0) return inferredByValues;

        // Fallback 2: legacy fields so uncategorized/older data still works.
        return LEGACY_VARIANT_IDS.map((id) => {
            const variant = variantTypes.find((vt) => Number(vt.id) === id);
            if (!variant) return null;
            return { variant_id: id, variant, is_required: false };
        }).filter(Boolean);
    }, [categoryVariantTypes, variantTypes, variantValues, form.category_id]);
    ...
    const fetchCategoryVariantTypes = async (categoryId) => {
        try {
            const res = await api.get("/supplier/category-variant-types", {
                params: { category_id: categoryId },
            });
            setCategoryVariantTypes(res.data?.data || []);
        } catch (e) {
            console.error("Failed to load category variant types:", e);
        }
    };
    ...
    const fetchVariantValues = async () => {
        try {
            ...
            const variantData =
                res.data?.data !== undefined ? res.data.data : res.data || [];
            const sizes = Array.isArray(variantData)
                ? variantData.find((v) => v.id === 1)?.values || []
                : [];
            const colors = Array.isArray(variantData)
                ? variantData.find((v) => v.id === 2)?.values || []
                : [];
            const weights = Array.isArray(variantData)
                ? variantData.find((v) => v.id === 3)?.values || []
                : [];
    ...
    const setupEditMode = (product) => {
        ...
        setAdditionalImagePreviews(
            (product.additional_images || []).map((img) => `/storage/${img}`),
        );
        ...
        productVariants.forEach((v, idx) => {
            ...
            if (v.existing_extra_images)
                extraPreviews[idx] = v.existing_extra_images.map(
                    (img) => `/storage/${img}`,
                );
        });
    ...
    const addVariant = () => {
        const attributes = resolvedCategoryVariantTypes.map((cvt) => ({
    ...
    const updateVariantAttribute = (idx, variantId, valueId) => {
        ...
        const normalizedValueId = valueId === "" ? "" : Number(valueId);
        ...
        setVariants((prev) =>
            prev.map((v, i) => {
                ...
                const foundIndex = attributes.findIndex(
                    (a) => Number(a.variant_id) === normalizedVariantId,
                );
                let updatedAttrs = attributes;

                if (foundIndex >= 0) {
                    updatedAttrs = attributes.map((a, attrIdx) =>
                        attrIdx === foundIndex
                            ? { ...a, variant_value_id: normalizedValueId }
                            : a,
                    );
                } else {
                    updatedAttrs = [
                        ...attributes,
                        {
                            ...
                            variant_value_id: normalizedValueId,
                        },
                    ];
                }
    ...
    const removeAdditionalImage = (imgIdx, isExisting = false) => {
        if (isExisting) {
            ...
            setForm((f) => ({
                ...f,
                additional_images: f.additional_images.filter(
                    (_, i) => i !== imgIdx,
                ),
            }));
            setAdditionalImagePreviews((prev) =>
    ...
    const removeVariantExtraImage = (vIdx, imgIdx, isExisting = false) => {
        if (isExisting) {
                    ...
                    i === vIdx
                        ? {
                              ...
                              existing_extra_images:
                                  v.existing_extra_images.filter(
                                      (_, j) => j !== imgIdx,
                                  ),
                          }
                        : v,
            ...
            setVariantExtraPreviews((prev) => {
                const updated = { ...prev };
                if (updated[vIdx])
                    updated[vIdx] = updated[vIdx].filter(
                        (_, j) => j !== imgIdx,
                    );
                return updated;
            });
    ...
    const handleSubmit = async (e) => {
        ...
        const validVariants = variants.filter((v) => {
            ...
            const hasLegacy =
                v.size?.trim() || v.color?.trim() || v.weight?.trim();
            return hasAttribute || hasLegacy || v.stock > 0;
        });
        if (variants.length > 0 && validVariants.length === 0) {
            showToast(
                "Please fill in at least one field for each variant or remove empty variants",
                "error",
            );
            return;
        }
        ...
        try {
            ...
            if (form.additional_images?.length > 0)
                form.additional_images.forEach((img) =>
            ...
            if (validVariants.length > 0) {
                ...
                validVariants.forEach((v, idx) => {
                    ...
                    if (variantExtraImages[idx]?.length > 0)
                        variantExtraImages[idx].forEach((img) =>
                            fd.append(`variant_extra_images_${idx}[]`, img),
                        );
                });
            }
            ...
            if (editing) {
                ...
                await api.post(`/supplier/products/${editing.id}`, fd, {
            ...
            <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                <div>
                    <Button
                        variant="ghost"
                        size="sm"
                        className="text-primary font-semibold mb-1 -ml-2"
                        ...
                        <Card className="p-5">
                            ...
                            <div className="space-y-3">
                                {variants.map((v, idx) => (
                                    <Card
                                        key={idx}
                                        className="p-4 bg-secondary/30 border-2"
                                    >
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                                {resolvedCategoryVariantTypes.map(
                                                    (cvt) => {
                                                        const variantType =
                                                            cvt.variant;
                                                        const currentValue =
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                Manage Gallery -{" "}
                                {imageModalTarget === "base"
                                    ? "Main Product"
                                    : `Variant #${Number(imageModalTarget) + 1}`}
                            </span>
                            <button
                                type="button"
                                onClick={() => setImageModalOpen(false)}
                                        ...
                                        if (imageModalTarget === "base") {
                                            isExisting =
                                                ...
                                                (existingAdditionalImages || [])
                                                    .length;
                                            preview =
                                                additionalImagePreviews[i];
                                        } else {
                                            const v =
                                                variants[imageModalTarget];
                                            isExisting = Boolean(
                                                v?.existing_extra_images &&
                                                v.existing_extra_images[i],
                                            );
                                            preview = (variantExtraPreviews[
                                                imageModalTarget
                                            ] || [])[i];
                                        }
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) =>
                                                                ...
                                                                "base"
                                                                    ...
                                                                    : handleVariantExtraImagesChange(
                                                                          imageModalTarget,
                                                                          e,
                                                                      )
                                                            }
