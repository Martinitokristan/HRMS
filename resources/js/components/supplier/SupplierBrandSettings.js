...
export default function SupplierBrandSettings() {
    ...
    const fetchAll = async (silent = false) => {
        ...
        try {
            ...
            const bData =
                brandsRes.data?.data !== undefined
                    ? brandsRes.data.data
                    : brandsRes.data;
            const cData =
                catsRes.data?.data !== undefined
                    ? catsRes.data.data
                    : catsRes.data;
    ...
    const toggleCategoryFor = (which, id) => {
        ...
        const ids = Array.isArray(target.category_ids)
            ? target.category_ids
            : [];
    ...
    const handleAddBrand = async (e) => {
        ...
        try {
            ...
            showToast(
                err.response?.data?.message || "Failed to add brand",
                "error",
            );
        } finally {
            setSaving(false);
        ...
        setEditing({
            ...
            category_ids: Array.isArray(b.category_ids)
                ...
                : Array.isArray(b.categories)
                  ? b.categories.map((c) => Number(c.id))
                  : [],
        });
    };
        ...
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="mb-6">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                    Create and manage brands for your products. Tag each brand
                    to one or more categories so the Brand dropdown filters
                    correctly when adding a product.
                </p>
            </div>
                    ...
                    <div className="mt-3">
                        <Label className="text-xs">
                            ...
                            <span className="text-muted-foreground">
                                (tap to toggle — leave empty for "shows in all
                                categories")
                            </span>
                        </Label>
            ...
            <Card className="overflow-hidden shadow-sm">
                {loading ? (
                    ...
                    <div className="text-center py-12 px-6">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                            Add your first brand above to start assigning brands
                            to your products.
                        </p>
                    </div>
                ) : (
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-secondary/50 hover:bg-secondary/50">
                                <TableHead className="text-[11px] font-bold uppercase tracking-wider px-4 w-10">
                                    #
                                </TableHead>
                                ...
                                <TableHead className="text-[11px] font-bold uppercase tracking-wider px-4 text-right">
                                    Actions
                                </TableHead>
                            </TableRow>
                        </TableHeader>
                        ...
                        <div className="flex items-start justify-between mb-3">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Update name, description, or which
                                    categories this brand appears under.
                                </p>
                            </div>
                            <button
                                onClick={() => !editSaving && setEditing(null)}
