...
export default function SupplierOrders({ mode = "completed" }) {
    ...
    const performAccept = async (order) => {
        ...
        try {
            ...
            showToast(
                `${order.po_number} accepted! You can now mark it as delivered when ready.`,
                "success",
            );
            markStale(
                STALE_KEYS.SUPPLIER_ORDERS,
                STALE_KEYS.SUPPLIER_DASHBOARD,
                STALE_KEYS.ADMIN_PURCHASES,
            );
            fetchOrders(true);
        } catch (err) {
            showToast(
                err.response?.data?.message || "Failed to accept PO",
                "error",
            );
        } finally {
            setSubmitting(false);
    ...
    const handleReject = async () => {
        ...
        try {
            await api.post(
                `/supplier/purchase-orders/${rejectModal.id}/reject`,
                {
                    rejection_reason: rejectionReason.trim(),
                },
            );
            showToast(
                `${rejectModal.po_number} rejected. The admin has been notified.`,
                "success",
            );
            markStale(
                STALE_KEYS.SUPPLIER_ORDERS,
            ...
            showToast(
                err.response?.data?.message || "Failed to reject PO",
                "error",
            );
        } finally {
            setSubmitting(false);
    ...
    const handleDeliver = async () => {
        ...
        try {
            await api.post(
                `/supplier/purchase-orders/${selectedOrder.id}/deliver`,
                {
            ...
            <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">
                        {isRequestMode
                            ? "Track and respond to new purchase requests from admin"
                            : "History of finalized purchase transactions"}
                    </p>
                </div>
            </div>
                    ...
                    <div className="space-y-4">
                            ...
                            <div className="flex items-start gap-2">
                                ...
                                <div>
                                    <div className="font-semibold text-sm mb-0.5">
                                        You are rejecting this Purchase Order.
                                    </div>
                                    <div className="text-sm text-muted-foreground">
                                        The admin will be notified with your
                        ...
                        <div>
                            ...
                            <div className="font-semibold text-foreground">
                                {rejectModal.po_number} —{" "}
                                {Array.isArray(rejectModal.items)
                                    ? rejectModal.items.length
                                    : 0}{" "}
                                items
                            </div>
                            <div className="text-sm text-muted-foreground">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="text-xs text-muted-foreground">
                                {rejectionReason.length} characters (minimum 10
                                required)
                            </div>
                        </div>
            ...
            <Modal
                ...
                title={
                    selectedOrder?.action === "deliver"
                        ...
                        : `Order Details - ${selectedOrder?.po_number}`
                }
                size="xl"
            >
                {selectedOrder && (
                        ...
                        {selectedOrder.action === "deliver" ? (
                            <div className="space-y-4">
                                ...
                                <p className="text-muted-foreground text-sm">
                                    You are marking order{" "}
                                    <strong className="text-foreground">
                                        {selectedOrder.po_number}
                                    </strong>{" "}
                                    as ready for delivery. Please confirm the
                                    items below are packed.
                                </p>
                                    ...
                                    {Array.isArray(selectedOrder?.items)
                                        ? selectedOrder.items.map((item) => {
                                              ...
                                              return (
                                                  <div
                                                      key={item.id}
                                                      className="flex justify-between items-start px-4 py-2.5"
                                                  >
                                                      <div>
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <h6 className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1.5">
                                            Purchaser
                                        </h6>
                                        <div className="font-bold text-sm text-foreground">
                                            HRMS Central Warehouse
                                        </div>
                                        <div className="text-sm text-muted-foreground">
