...
export default function ProductForm({
    ...
    const sizeVariant = Array.isArray(variants)
        ? variants.find((v) => v.name?.toLowerCase() === "size")
        : null;
    const colorVariant = Array.isArray(variants)
        ? variants.find((v) => v.name?.toLowerCase() === "color")
        : null;
    const weightVariant = Array.isArray(variants)
        ? variants.find(
              (v) =>
                  v.id === 3 ||
    ...
    const getVariantLabelById = (id, type) => {
        ...
        let variantList = [];
        if (type === "size") variantList = sizeVariant?.values || [];
        if (type === "color") variantList = colorVariant?.values || [];
        if (type === "weight") variantList = weightVariant?.values || [];

        const variant = Array.isArray(variantList)
            ? variantList.find((v) => v.id === parseInt(id))
            : null;
        return variant
            ? variant.label
    ...
    useEffect(() => {
        ...
        if (product) {
            ...
            const pvs = Array.isArray(product.product_variants)
                ...
                setVariantRows(
                    pvs.map((pv) => {
                        return {
                            ...
                            size_value:
                                pv.size_value?.label ||
                                getVariantLabelById(pv.size_value_id, "size"),
                            color_value:
                                pv.color_value?.label ||
                                getVariantLabelById(pv.color_value_id, "color"),
                            weight_value:
                                pv.weight_value?.label ||
                                getVariantLabelById(
                                    pv.weight_value_id,
                                    "weight",
                                ),
    ...
    const handleSubmit = async (e) => {
        ...
        const variantList =
            variantEnabled && Array.isArray(variantRows)
                ? variantRows.map((row, index) => {
                      ...
                      return {
                          size_value_id:
                              row.size_value !== "Regular"
                                  ? sizeVariant?.values?.find(
                                        (v) => v.label === row.size_value,
                                    )?.id || null
                                  : null,
                          color_value_id:
                              row.color_value !== "Regular"
                                  ? colorVariant?.values?.find(
                                        (v) => v.label === row.color_value,
                                    )?.id || null
                                  : null,
                          weight_value_id:
                              row.weight_value !== "Regular"
                                  ? weightVariant?.values?.find(
                                        (v) => v.label === row.weight_value,
                                    )?.id || null
                                  : null,
                          stock: row.stock || 0,
                          price_override:
                        ...
                        <TabsContent value="variants" className="space-y-4">
                            <div className="flex items-center justify-between">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-sm text-gray-500">
                                        View product variants and set sale
                                        percentages
                                    </p>
                                </div>
                            </div>
                            ...
                            variantRows.length > 0 ? (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-4 py-2">
                                                            <div className="relative">
                                                                ...
                                                                />
                                                                <span className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-500 text-[10px]">
                                                                    %
                                                                </span>
                                                            </div>
                                                        </td>
                                ...
                                <div className="text-center py-12 text-muted-foreground bg-secondary/20 rounded-xl border border-dashed">
                                    ...
                                    <p className="text-xs opacity-60 mt-1">
                                        This product is managed as a single
                                        item.
                                    </p>
                                </div>
                            )}
