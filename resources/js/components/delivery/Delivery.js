...
export default function Delivery() {
    ...
    const handleAssign = async (deliveryId, riderId) => {
        if (!riderId) return;
        try {
            await api.put(`/deliveries/${deliveryId}/assign`, {
                rider_id: riderId,
            });
        ...
        try {
            await Promise.all(
                selectedDeliveries.map((id) =>
                    api.put(`/deliveries/${id}/assign`, {
                        rider_id: bulkRiderId,
                    }),
                ),
            );
            showToast(
                `Assigned ${selectedDeliveries.length} deliveries to rider`,
            );
            markStale(STALE_KEYS.ADMIN_DELIVERIES, STALE_KEYS.RIDER_DASHBOARD);
            fetchData(true);
            ...
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">
                        Track, assign and manage all deliveries in real-time
                    </p>
                </div>
                <Button
                    variant="outline"
            ...
            {selectedDeliveries.length > 0 && (
                <Card className="p-3 mb-4 flex items-center justify-between flex-wrap gap-3 bg-primary/5 border-primary/20">
                    <span className="text-sm font-semibold text-foreground">
                        {selectedDeliveries.length} delivery(s) selected
                    </span>
                    <div className="flex items-center gap-2">
                        {!showBulkAssign ? (
                            ...
                            <div className="flex items-center gap-2">
                                <select
                                    className="flex h-8 rounded-md border border-input bg-transparent px-2 py-1 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                                    value={bulkRiderId}
                                    onChange={(e) =>
                                        setBulkRiderId(e.target.value)
                                    }
                                >
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    {[
                        ...
                        <Button
                            key={key}
                            variant={
                                statusFilter === key ? "default" : "outline"
                            }
                            size="sm"
                            onClick={() => {
                                setStatusFilter(key);
            ...
            <div className="flex items-center justify-between mb-2 px-1">
                ...
                {riders.length > 0 && (
                    <span className="text-sm text-muted-foreground flex items-center gap-1">
                        <Truck className="h-3.5 w-3.5" /> {riders.length} riders
                        available
                    </span>
                )}
            </div>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell className="px-4 py-3">
                                            ...
                                            <div
                                                className="text-[12px] text-muted-foreground truncate max-w-[250px]"
                                                title={d.address}
                                            >
                                                {d.address}
                                            </div>
                                                    ...
                                                    >
                                                        <Button
                                                            ...
                                                            onClick={() =>
                                                                showConfirm(
                                                                    "Delete Delivery",
                                                                    "Are you sure you want to delete this delivery? Only pending deliveries can be deleted.",
                                                                    () =>
