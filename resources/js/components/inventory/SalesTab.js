...
export default function SalesTab() {
    ...
    const performUpdateStatus = async (saleId, newStatus) => {
        try {
            const res = await api.put(`/sales/${saleId}/status`, {
                status: newStatus,
            });
                ...
                {viewOrder && (
                    <div className="space-y-5">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            {viewOrder.payment_method === "gcash" &&
                                viewOrder.payment_phone_number && (
                                    <Card className="bg-blue-50 border-blue-200 p-4 sm:col-span-2">
                                        ...
                                        {/* Expiry SMS warning */}
                                        {viewOrder.payment_expiry_sms_sent_at &&
                                            viewOrder.status ===
                                                "pending_payment" && (
                                        ...
                                        {viewOrder.payment_proof_path && (
                                            <div className="mt-3 pt-3 border-t border-blue-200/50 space-y-3">
                                                ...
                                                <a
                                                    href={`/storage/${viewOrder.payment_proof_path}`}
                                                    target="_blank"
                                                    rel="noreferrer"
                                                    className="block w-32 h-32 rounded-lg overflow-hidden border border-blue-200 hover:opacity-90 transition-opacity"
                                                >
                                                    <img
                                                        src={`/storage/${viewOrder.payment_proof_path}`}
                                                        alt="GCash Receipt"
                        ...
                        {getNextStatuses(viewOrder.status).length > 0 && (
                            <div className="flex gap-3">
                                {getNextStatuses(viewOrder.status).map(
                                    (nextStatus) => {
                                        const config =
                        ...
                        {viewOrder.status !== "returned" &&
                            ...
                            viewOrder.status === "delivered" && (
                                <Button
                                    variant="destructive"
                                    className="w-full"
                                    onClick={() => handleReturn(viewOrder.id)}
                                >
...
function SalesReceiptModal({ order, onClose, settings }) {
    ...
    const itemsSubtotal =
        order.items?.reduce((sum, i) => sum + Number(i.subtotal || 0), 0) || 0;
        ...
        >
            ...
            >
                ...
                <div className="bg-[#f3f4f6] px-6 py-[18px] flex flex-col items-center text-center shrink-0 border-b border-[#e5e7eb]">
                    <h1 className="text-[18px] font-bold text-[#111827] tracking-[0.02em]">
                        {storeName}
                    </h1>
                ...
                <div className="overflow-y-auto flex-1">
                    ...
                    <div className="flex gap-4 px-6 py-[14px] bg-[#f9fafb] border-b border-[#e5e7eb]">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-[14px] font-semibold text-[#f97316]">
                                {fmtDate(order.delivery?.delivered_at)}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3 px-6 py-3 border-b border-[#e5e7eb]">
                        <div>
                            ...
                            {order.customer?.email && (
                                <p className="text-[11px] text-[#f97316] mt-0.5">
                                    {order.customer.email}
                                </p>
                            )}
                        ...
                        <table className="w-full text-[13px] border-collapse">
                            <thead>
                                <tr className="bg-[#f3f4f6]">
                                    <th className="text-left px-[10px] py-[7px] text-[10px] font-medium text-[#6b7280] uppercase tracking-[0.05em]">
                                        Item Description
                                    </th>
                                    <th className="text-center px-[10px] py-[7px] text-[10px] font-medium text-[#6b7280] uppercase tracking-[0.05em]">
                                        QTY
                                    </th>
                                    <th className="text-center px-[10px] py-[7px] text-[10px] font-medium text-[#6b7280] uppercase tracking-[0.05em]">
                                        Unit Price
                                ...
                                {(order.items || []).map((item, idx) => {
                                    const variantLabel = item.product_variant
                                        ? [
                                              item.product_variant.size_value
                                                  ?.label,
                                              item.product_variant.color_value
                                                  ?.label,
                                              item.product_variant.weight_value
                                                  ?.label,
                                          ]
                                              .filter(Boolean)
                                              .join(" / ")
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="px-[10px] py-[10px] text-center text-[#6b7280]">
                                                {fmtMoney(item.unit_price)}
                                            </td>
                                            <td className="px-[10px] py-[10px] text-center font-bold text-[#111827]">
                    ...
                    <div className="flex justify-end px-6 pb-4 pt-2.5">
                        <div className="w-52 space-y-1.5">
                            ...
                            <div className="border-t border-[#e5e7eb] pt-2 flex justify-between items-baseline gap-2">
                                <span className="text-[12px] font-bold text-[#111827] uppercase">
                                    Grand Total
                                </span>
                    ...
                    <div className="px-6 py-[10px] bg-[#f9fafb] border-t border-[#e5e7eb]">
                        <p className="text-[11px] text-[#9ca3af] italic text-center">
                            Thank you for your purchase! Please keep this
                            receipt for your records.
                            {storeEmail
                                ? ` For inquiries, contact ${storeEmail}.`
                                : ""}
                        </p>
                    </div>
                </div>
