...
function IphoneDatePicker({ value, onChange }) {
    ...
    const pick = (cell) => {
        const d = new Date(
            year +
                (cell.otherMonth === 1 && month === 11 ? 1 : 0) +
                ...
                <div className="absolute z-50 mt-2 left-0 w-[300px] rounded-2xl border border-gray-200 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.15)] p-4 animate-in fade-in zoom-in-95 duration-150 origin-top-left">
                    ...
                    */}
                    <div className="flex items-center justify-between mb-2">
                        {mode === "years" ? (
                            <div className="flex-1 text-center text-[15px] font-bold text-gray-900">
                                Select Year
                            </div>
                        ) : mode === "months" ? (
...
function CashRemittanceTab() {
    ...
    const [date, setDate] = useState(getTodayInManila());
    ...
    const load = useCallback(async () => {
        setLoading(true);
        try {
            const r = await api.get("/admin/cash-remittance", {
                params: { date },
            });
            setRows(r.data?.data || []);
        } catch (e) {
    ...
    const markRemitted = async (riderId) => {
        ...
        try {
            await api.post(
                `/admin/cash-remittance/${riderId}/${date}/mark-remitted`,
            );
                    ...
                    <tbody className="divide-y divide-border">
                        {rows.length === 0 && !loading && (
                            <tr>
                                ...
                                >
                                    No COD deliveries on this date.
                                </td>
                            </tr>
                        )}
                        {rows.map((r) => (
                            ...
                            >
                                ...
                                <td className="px-4 py-3 text-right font-mono">
                                    {formatPHP(r.expected)}
                                </td>
                                <td className="px-4 py-3 text-right font-mono text-green-600">
                                    {formatPHP(r.remitted)}
                                </td>
                                <td className="px-4 py-3 text-right font-mono">
                                    {r.outstanding > 0 ? (
                                        <span className="text-red-600 font-bold">
                                            {formatPHP(r.outstanding)}
                                        </span>
                                    ) : (
...
function RiderPayoutsTab() {
    ...
    const release = async (deliveryId, decision) => {
        ...
        try {
            await api.post("/admin/payouts/release-hold", {
                delivery_id: deliveryId,
                decision,
            });
            showToast(
                decision === "release" ? "Released to eligible" : "Rejected",
                "success",
            );
            await load();
        } catch (e) {
            ...
            <Tabs value={status}>
                <TabsList>
                    <TabsTrigger
                        ...
                        className="gap-1.5"
                    >
                        <Wallet className="h-3.5 w-3.5" /> Eligible
                    </TabsTrigger>
                    <TabsTrigger
                        value="held"
                        ...
                        <div className="flex items-center justify-between bg-gray-50 px-4 py-3 border-b border-border">
                            <div>
                                <div className="font-bold text-foreground">
                                    {g.rider_name || `Rider #${g.rider_id}`}
                                </div>
                                <div className="text-xs text-muted-foreground">
                                    {g.count} delivery{g.count === 1 ? "" : "s"}{" "}
                                    • {formatPHP(g.total_fee)}
                                </div>
                            </div>
                            {status === "eligible" && (
                                <Button
                                        ...
                                        <td className="px-4 py-2">
                                            <div className="flex gap-1.5">
                                                {row.geofence_flagged && (
                                                    ...
                                                    >
                                                        Geofence{" "}
                                                        {row.geofence_distance !=
                                                        null
                                                            ? `${row.geofence_distance}m`
                                                            : ""}
                                                    </Badge>
                                                )}
                                                {row.customer_disputed && (
                                                    <Badge
                                                        variant="destructive"
                                                        className="text-[10px]"
                                                    >
        ...
        <div className="relative">
            ...
            <Tabs value={tab} className="mb-6 relative z-0">
                <TabsList>
                    ...
                    <TabsTrigger
                        value="payouts"
                        onClick={() => navigate("/payouts/riders")}
                        className="gap-2"
                    >
