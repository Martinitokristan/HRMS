...
export default function GCashLogs() {
    ...
    const fetchLogs = async () => {
        setLoading(true);
        try {
            const url = new URL('/api/admin/gcash-logs', window.location.origin); // Assuming we can use full path or just let interceptor handle it
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                        Monitor incoming GCash SMS webhooks and algorithm matching status.
                    </p>
                </div>
                ...
                <div className="flex bg-white rounded-lg border border-slate-200 p-1">
                    ...
                    <button
                        onClick={() => { setFilter('matched'); setPage(1); }}
                    ...
                    <button
                        onClick={() => { setFilter('unmatched'); setPage(1); }}
                    ...
                    <button
                        ...
                        title="Refresh"
                    >
                        <RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin text-[#007DFE]' : ''}`} />
                    </button>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-50">
                            <tr>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap">
                                    Date & Time
                                </th>
                                ...
                                <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm text-slate-700 min-w-[300px]">
                                        <div className="bg-slate-100 p-2 rounded text-xs break-words break-all border border-slate-200 max-w-sm">
                                            {log.sms_body ? cleanSmsBody(log.sms_body) : <span className="text-slate-400 italic">No message</span>}
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        {log.sale ? (
                                            <a href={`/dashboard?order=${log.sale.order_number}`} className="text-[#007DFE] hover:underline">
