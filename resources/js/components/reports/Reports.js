            ...
            <div className="chart-svg-container">
                <div className="horizontal-bar-chart">
                    {sortedData.map((product, idx) => {
                        const widthPct =
                            (Number(product.total_sold) / maxValue) * 100;
                        return (
                                ...
                                <div className="bar-track">
                                    ...
                                    >
                                        <span className="bar-value">
                                            {formatValue(product.total_sold)}
                                        </span>
                                    </div>
                                </div>
                            </div>
    ...
    <Card className="p-4">
        <div className="flex items-center gap-3">
            ...
            <div>
                ...
                {trend && (
                    <div
                        className={`text-xs font-semibold ${trendUp ? "text-success-foreground" : "text-destructive"}`}
                    >
...
export default function Reports() {
    ...
    const formatNumber = (val) =>
        new Intl.NumberFormat("en-PH").format(val || 0);
    ...
    const downloadPDF = async () => {
        ...
        try {
            ...
            if (
                ...
                link.setAttribute(
                    "download",
                    `sales_report_${period}_${new Date().toISOString().split("T")[0]}.pdf`,
                );
            ...
            if (e.response && e.response.data) {
                ...
                alert(
                    "Failed to download PDF report. Please check your connection and try again.",
                );
            }
        } finally {
            setExporting(false);
            ...
            <div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    {[1, 2, 3, 4].map((i) => (
                        <div
                            key={i}
                            className="h-[120px] rounded-xl bg-secondary animate-pulse"
                        />
                    ))}
                </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <MiniStat
                                ...
                                value={`${Math.max(...chartData.map((d) => d.orders), 0)} orders`}
                                icon={<Target className="h-4 w-4" />}
                            />
                            <MiniStat
                                label="Active Days"
                                value={`${chartData.filter((d) => d.value > 0).length} days`}
                ...
                {activeTab === "products" && (
                    <div className="space-y-6">
                        ...
                        <h3 className="text-base font-bold text-foreground flex items-center gap-2">
                            <Trophy className="h-4 w-4 text-amber-500" /> Top
                            Performing Products
                        </h3>
                            ...
                            {topProducts.length === 0 ? (
                                <Card className="p-8 text-center">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        No product data available for this
                                        period
                                    </p>
                                </Card>
                            ) : (
